...
protocol Renderer {
    /// Moves the pen to `p` without drawing anything.
    func moveTo(_ p: CGPoint)
    
    /// Draws a line from the pen's current position to `p`, updating
    /// the pen position.
    func lineTo(_ p: CGPoint)
    
    /// Draws the fragment of the circle centered at `center` having the given
...
struct Bubble : Drawable {
    func draw(renderer: Renderer) {
        renderer.circleAt(center: center, radius: radius)
        renderer.circleAt(center: highlightCenter, radius: highlightRadius)
    }
...
//        && !zip(lhs.elements, rhs.elements).contains { !$0.isEqualTo($1) }
    ...
    return lhs.elements.count == rhs.elements.count
        && !zip(lhs.elements, rhs.elements).contains { !$0.0.isEqualTo(other: $0.1) }
}
...
func sampleDiagram(frame: CGRect) -> Diagram {
    ...
    sample.add(circle)
    sample.add(regularPolygon(n: 3, center: center, radius: r))
...
showCoreGraphicsDiagram(title: "Diagram") { sample.draw(renderer: $0) }
